...
public class PlayerMovement : MonoBehaviour
{
    ...
    void NormalStateUpdate()
    {
        ...
        if (isNearClimbableWall && GetJumpButton())
        {
        ...
        if (!inDialogue && !isGrounded && rb.linearVelocity.y < 0 && GetJumpButton() && canGrabLedge)
        {
            CheckForLedge();
        }
    }
    ...
    void ClimbingStateUpdate()
    {
        ...
        if (!inDialogue && (GetJumpButtonDown() || !isNearClimbableWall))
        {
            ExitClimbing();
        }
    }
    ...
    void LedgeGrabStateUpdate()
    {
        ...
        if (!inDialogue && GetJumpButtonDown())
        {
            StartPullUp();
        }
    ...
    public void MobileJump()
    {
        ...
        // Try to pull up if grabbing ledge
        else if (currentState == PlayerState.LedgeGrab)
        {
            StartPullUp();
        }
        // Try to exit climbing if climbing
